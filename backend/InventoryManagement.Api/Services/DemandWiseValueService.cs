...
namespace InventoryManagement.Api.Services
{
    public class DemandWiseValueService : IDemandWiseValueService
    {
        ...
        {
            var response = new DemandWiseValueResponse();
...
ORDER BY ie.IssuedDate DESC, ie.DRNo DESC;";
            ...
            try
            {
                ...
                response.Totals = new DemandWiseValueTotals
                {
                    TotalIssuedQty = response.Records.Sum(record => record.IssuedQty),
                    TotalUnitBuyingPrice = response.Records.Sum(record => record.UnitBuyingPrice),
                    TotalBuyingPrice = response.Records.Sum(record => record.TotalBuyingPrice)
                };
            }
            catch (Exception ex)
            {
