...
namespace InventoryManagement.Api.Controllers
{
    ...
    public class RacksController : BaseController
    {
        ...
        public RacksController(IUserSessionCacheService sessionCache, IRackService rackService, ILogger<RacksController> logger)
            : base(sessionCache)
        {
        ...
        public async Task<ActionResult<IEnumerable<Rack>>> GetAll()
        {
            try
            {
                ...
                var racks = await _rackService.GetAllRacksAsync(branchId);
                return Ok(racks);
            }
            catch (Exception ex)
            {
