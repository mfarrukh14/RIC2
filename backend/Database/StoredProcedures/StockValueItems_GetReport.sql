...
BEGIN
    ...
    -- was silently dropping those rows entirely. Name resolved via
    -- DenormalizedItemName the same way GRNItems already falls back elsewhere
    -- in this schema.
    --
    -- Store resolved from grn.StoreId directly, not via
    -- PurchaseOrderId -> Inv.PurchaseOrders.StoreId: PurchaseOrderId is NULL
    -- on all but a handful of GRNs, so the PO-based path made every row
    -- show up as "Unassigned Store" regardless of the GRN's real store.
    ;WITH Filtered AS (
        SELECT
