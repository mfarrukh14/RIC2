...
const ItemWiseSalePage = () => {
  ...
  // Item dropdown only populates (with live per-store stock qty) once a store is
  // selected - matches the same pattern as Retail Pharmacy's item picker.
  useEffect(() => {
    if (!storeId || !branchId) {
      setItems([]);
      setItemId('');
    ...
    pharmacyApi.getActiveItems(branchId, storeId)
      ...
      .catch((itemsError) => console.error('Error loading items:', itemsError));
        ...
        <section className="rounded-md border border-slate-200 bg-white shadow-sm">
          ...
          <div className="grid grid-cols-1 gap-4 px-6 py-5 lg:grid-cols-5">
            <div>
              ...
              <select value={storeId} onChange={(event) => { setStoreId(event.target.value); setItemId(''); }} className="w-full rounded-md border border-slate-200 px-3 py-2.5 text-sm outline-none focus:border-indigo-400">
            ...
            <div>
              ...
              <select
                ...
                disabled={!storeId}
                className="w-full rounded-md border border-slate-200 px-3 py-2.5 text-sm outline-none focus:border-indigo-400 disabled:bg-slate-100"
              >
                <option value="">{storeId ? 'All Items' : 'Select a store first'}</option>
                {items.map((item) => (
                  <option key={item.itemId} value={item.itemId}>
                    {item.itemName} - Stock {formatCurrency(item.storeStockQty)}
                  </option>
                ))}
              </select>
            </div>
            ...
            <div className="flex items-end">
              <button type="button" onClick={loadEntries} className="w-full rounded-md bg-indigo-600 px-4 py-2.5 text-sm font-medium text-white hover:bg-indigo-700">Generate Report</button>
            </div>
          </div>
            ...
            <table className="min-w-full border-separate border-spacing-0 text-sm">
              <thead>
                <tr className="text-left text-slate-700">
                  ...
                  <th className="border-y border-slate-200 bg-white px-4 py-3 font-semibold">Item</th>
              ...
              <tbody>
                {loading ? (
                  <tr><td colSpan="9" className="border-b border-slate-200 px-4 py-10 text-center text-slate-500">Loading...</td></tr>
                ) : pageItems.length === 0 ? (
                  <tr><td colSpan="9" className="border-b border-slate-200 px-4 py-10 text-center text-slate-500">No data available in table</td></tr>
                ) : (
                  pageItems.map((entry, index) => (
                    <tr key={`${entry.challanNo}-${index}`} className="text-slate-700">
                      ...
                      <td className="border-b border-slate-200 px-4 py-3">{entry.itemName}</td>
                      <td className="border-b border-slate-200 px-4 py-3">{entry.quantity}</td>
                      <td className="border-b border-slate-200 px-4 py-3">{formatCurrency(entry.unitPrice)}</td>
              ...
              {filteredEntries.length > 0 && (
                <tfoot>
                  <tr className="text-left font-semibold text-slate-800">
                    ...
                    <td className="border-t border-slate-200 px-4 py-3">{totalQuantity}</td>
                    <td className="border-t border-slate-200 px-4 py-3"></td>
                    <td className="border-t border-slate-200 px-4 py-3">{formatCurrency(totalSale)}</td>
                  </tr>
                </tfoot>
              )}
            </table>
...
export default ItemWiseSalePage;
